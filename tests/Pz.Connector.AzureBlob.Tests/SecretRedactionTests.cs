...
/// <summary>Azure variant of <c>Pz.Connector.S3.Tests.SecretRedactionTests</c>: the
/// <c>connection_string</c> connection value must never leak outside the CREATE SECRET statement text
/// itself -- not into a <see cref="PlannedNode.Reason"/>, not into plan.json, and not into a <see
/// cref="NativeSetup"/> failure message even when the setup statement itself is malformed (a malformed
/// CREATE SECRET is exactly the shape the redaction protects against -- see <see
/// cref="Pz.Engine.Execution.NativeStatementRedactor"/>).</summary>
public sealed class SecretRedactionTests
{
    private const string SecretValue = "AZ_SECRET_VALUE";
    ...
    public async Task Plan_reason_and_plan_json_never_contain_the_secret()
    {
        ...
        registry.AddSink("azureblob", new AzureConnector());
        ...
        var sinkDef = new ConnectionDef("lake", "azureblob",
            new Dictionary<string, object?> { ["auth"] = "connection_string", ["connection_string"] = $"AccountKey={SecretValue}" }, [],
            "connections.yml") { Outputs = [new OutputDef("out", "stg_orders", "replace", "fail_on_change",
                new Dictionary<string, object?> { ["container"] = "lake", ["format"] = "parquet" })] };
        ...
        var dir = Path.Combine(Path.GetTempPath(), "pz-azure-redaction-tests", Guid.NewGuid().ToString("N"));
        try
        {
    ...
    {
        await using var sink = await ((ISinkConnector)new AzureConnector()).OpenAsync(
            new ConnectorConfig(new Dictionary<string, object?>
            {
                ["auth"] = "connection_string",
                ["connection_string"] = $"AccountKey={SecretValue}",
        ...
        // "connection_string" clause makes DuckDB reject the statement with a "LINE 1: ..." context block
        // that echoes the whole offending statement -- including the secret literal -- verbatim. This
        // reproduces deterministically without any network access (the parser rejects the statement before
        // "type azure" is ever resolved, so the azure extension need not even be installed).
        var broken = realSecretStatement.Replace("(type azure, connection_string", "(type azure connection_string", StringComparison.Ordinal);
