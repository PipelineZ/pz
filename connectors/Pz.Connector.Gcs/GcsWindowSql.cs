...
/// <summary>Windowed-bound pushdown for gcs native scan fragments. Replicated (not shared) from
/// <c>Pz.Connector.S3.S3WindowSql.Wrap</c> — connector projects deliberately do not reference each
/// other or Pz.Core, so this is the source-of-truth predicate shape and escaping copied
/// verbatim-in-spirit; keep in lockstep with S3WindowSql/AzureWindowSql/LocalFilesWindowSql if that
/// shape ever changes.</summary>
internal static class GcsWindowSql
{
    ...
    public static string Wrap(string fragment, DatasetSpec spec)
    {
        ...
        if (spec.WatermarkValue is null)
        {
            throw new InvalidOperationException(
                "GcsWindowSql.Wrap: WatermarkCursor/WatermarkUpperBound are set but WatermarkValue is " +
