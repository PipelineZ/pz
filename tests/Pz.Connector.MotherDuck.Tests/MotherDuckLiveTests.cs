...
public sealed class MotherDuckLiveTests : IDisposable
{
    ...
    public async Task Append_replace_merge_and_windowed_reads_round_trip_through_motherduck()
    {
        ...
        try
        {
            ...
            // branch and both insert — MERGE's key-uniqueness contract is the caller's, not something
            // this connector's plain generated SQL enforces. This is a live-observed finding, not the
            // brief's assumption (which expected the pair to collapse to one row); the connector's
            // generated SQL is unchanged since the docs make no dedup promise either.
