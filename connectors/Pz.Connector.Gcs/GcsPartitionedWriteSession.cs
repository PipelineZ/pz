...
/// <summary>Date-partitioned (fan-out), per-partition-atomic write session: routes each incoming
/// Arrow row into the output folder its <c>partition_by</c> timestamp renders to
/// (<see cref="PathTemplate.Render(string, DateTimeOffset)"/>) and drives one inner
/// <see cref="GcsWriteSession"/> per distinct folder — so a batch spanning two days lands as two
/// objects. Replicated (not shared) from <c>Pz.Connector.AzureBlob.AzurePartitionedWriteSession</c>
/// per the no-cross-connector-reference rule. Each inner session keeps its own
/// spool-then-atomic-upload commit-xor-abort state machine, independent of the others; the overall
/// write is therefore at-least-once at the SET level (a failed run may leave a subset of partitions
/// committed — <c>pz retry</c>/re-run reconciles), never falsely atomic across partitions: a staged
/// all-or-nothing promote is deliberately not used, because on an object store it can only pretend
/// to be atomic.
///
/// The fan-out session honours the same commit-xor-abort contract as a single session: exactly one
/// of <see cref="CommitAsync"/>/<see cref="AbortAsync"/> runs, after which writes/commits/aborts are
/// rejected; <see cref="DisposeAsync"/> always releases every opened inner session best-effort,
/// never masking an earlier failure with cleanup fallout.
///
/// Batch ownership: the batch handed to <see cref="WriteBatchAsync"/> is engine-owned only until
/// the call returns. Every per-folder slice is materialised SYNCHRONOUSLY (copied out of the incoming
/// batch into pooled, off-heap builder buffers via <see cref="ArrowBatchBuilder"/>) BEFORE the first
/// inner <c>WriteBatchAsync</c> await, so the engine-owned batch is never read across an await
/// boundary.</summary>
internal sealed class GcsPartitionedWriteSession : ISinkWriteSession
{
    ...
    internal GcsPartitionedWriteSession(
    ...
    public async ValueTask WriteBatchAsync(RecordBatch batch, CancellationToken ct)
    {
        ...
        try
        {
            ...
            //    grouping). The writes themselves then run concurrently, bounded so a batch spanning many
            //    partitions doesn't fan out an unbounded number of in-flight requests.
    ...
    public async ValueTask<WriteResult> CommitAsync(CancellationToken ct)
    {
        ...
        // Committed up-front enforces that even if an inner commit throws part-way -- the already-uploaded
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        // thing: an inner never committed aborts (spool reclaimed), a committed inner no-ops, a
        // commit-attempted-but-failed inner releases only local resources. Suppress cleanup fallout
        // unconditionally.
        foreach (var session in _byFolder.Values)
        {
            try
    ...
    /// timestamp/date by <c>GcsSink</c> before this session was created; a null partition value cannot be
