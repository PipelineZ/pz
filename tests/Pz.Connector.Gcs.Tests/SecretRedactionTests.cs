...
/// <summary>The <c>secret</c> connection value must never leak outside the CREATE SECRET statement
...
/// <see cref="Pz.Engine.Execution.NativeStatementRedactor"/>). Mirrors the s3 suite.</summary>
public sealed class SecretRedactionTests
{
    private const string SecretValue = "HMAC_SECRET_VALUE";
    ...
    public async Task Plan_reason_and_plan_json_never_contain_the_secret()
    {
        ...
        registry.AddSink("gcs", new GcsConnector());
        ...
        var sinkDef = new ConnectionDef("lake", "gcs",
            new Dictionary<string, object?> { ["auth"] = "hmac", ["key_id"] = "GOOGHMAC_TEST", ["secret"] = SecretValue }, [],
        ...
        var dir = Path.Combine(Path.GetTempPath(), "pz-gcs-redaction-tests", Guid.NewGuid().ToString("N"));
        try
        {
    ...
    {
        await using var sink = await ((ISinkConnector)new GcsConnector()).OpenAsync(
        ...
        // Reproduces the exact DuckDB Parser Error shape validated at the engine level: dropping the
        // comma before the "secret" clause makes DuckDB reject the statement with a "LINE 1: ..."
        // context block that echoes the whole offending statement -- including the secret literal --
        // verbatim. Deterministic without any network access (the parser rejects the statement before
        // "type gcs" is ever resolved, so httpfs need not even be installed).
