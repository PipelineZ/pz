...
/// table accepts only bulk CREATE/INSERT, no row-level DML), all executed by the remote server.
/// Commit semantics belong to the server, so Transactional is not declared.</summary>
