...
/// <see cref="GcsSource.GetSchemaAsync"/>'s contract-is-the-schema answer) and to DuckDB native-scan
/// type names (for the strict <c>columns = {…}</c> map). Replicated (not shared) from
/// <c>Pz.Connector.S3.S3TypeNameMap</c> per the no-cross-connector-reference rule — the fixed v0
/// type matrix, kept in lockstep with LocalFiles/Azure/S3 if it ever grows.</summary>
internal static class GcsTypeNameMap
{
