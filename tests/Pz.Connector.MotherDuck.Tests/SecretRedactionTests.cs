...
/// <summary>The token rides a `set motherduck_token = '…'` statement, never the attach string: it may
/// not reach a <see cref="PlannedNode.Reason"/>, plan.json, or a <see cref="NativeSetup"/> failure
/// message -- even when the carrier statement itself is malformed and DuckDB's parser echoes it.</summary>
public sealed class SecretRedactionTests
{
    private const string MotherDuckToken = "M0THER_T0KEN";

    private static Dictionary<string, object?> Connection() => new() { ["database"] = "lake", ["token"] = MotherDuckToken };

    private static CompiledDag Dag()
    {
        var connection = Connection();
        var sourceDef = new ConnectionDef("wh", "motherduck", connection,
    ...
    public async Task Plan_reason_and_plan_json_never_contain_the_token()
    {
        ...
        Assert.All(plan.Nodes, n => Assert.DoesNotContain(MotherDuckToken, n.Reason, StringComparison.Ordinal));

        var dir = Path.Combine(Path.GetTempPath(), "pz-motherduck-redaction-tests", Guid.NewGuid().ToString("N"));
        try
        {
            PlanWriter.Write(plan, dir);
            var json = await File.ReadAllTextAsync(Path.Combine(dir, "plan.json"));
            Assert.DoesNotContain(MotherDuckToken, json, StringComparison.Ordinal);
        }
        finally
        {
    ...
    {
        await using var source = await ((ISourceConnector)new MotherDuckConnector()).OpenAsync(new ConnectorConfig(Connection()), CancellationToken.None);
        ...
        Assert.DoesNotContain(MotherDuckToken, scan!.SetupStatements[^1], StringComparison.Ordinal);
        var carrier = Assert.Single(scan.SetupStatements, s => s.Contains(MotherDuckToken, StringComparison.Ordinal));

        // Break the carrier's grammar ("set" -> "sett") so DuckDB's parser rejects it with a
        // LINE-context block echoing the statement verbatim, before the token is ever applied --
        // deterministic and offline, so no PZ_TESTS_OFFLINE gate.
        var broken = carrier.Replace("set ", "sett ", StringComparison.Ordinal);
        ...
        try
        {
            ...
            Assert.DoesNotContain(MotherDuckToken, ex.Message, StringComparison.Ordinal);
        }
        finally
        {
            try { Directory.Delete(dir, recursive: true); } catch { /* best-effort cleanup */ }
