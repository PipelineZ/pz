...
public sealed class DuckDbPlannerTests : IDisposable
{
    ...
    private CompiledDag DuckDbToDuckDbDag(string mode = "replace", string outputName = "events_out")
    {
        ...
        var sinkDef = new ConnectionDef("mart", "duckdb", Connection(), [], "connections.yml")
        {
            Outputs = [output],
        };
    ...
    [Theory]
    [InlineData("append", "duckdb insert")]
    [InlineData("replace", "duckdb create-or-replace")]
    [InlineData("merge", "duckdb merge")]
    public async Task Both_directions_plan_onto_the_native_tier(string mode, string mechanism)
    {
        var plan = await new ExecutionPlanner(Registry())
            .PlanAsync(DuckDbToDuckDbDag(mode), forceUniversal: false, CancellationToken.None);

        var load = Assert.Single(plan.Nodes, n => n.Kind == NodeKind.SourceLoad);
        Assert.Equal(EdgeStrategy.NativeScan, load.Strategy);
        Assert.Contains("attach", load.Reason, StringComparison.Ordinal);
    ...
    public async Task Force_universal_is_PZ0312_in_both_directions()
    {
        var ex = await Assert.ThrowsAsync<PzValidationException>(
            () => new ExecutionPlanner(Registry())
                .PlanAsync(DuckDbToDuckDbDag(), forceUniversal: true, CancellationToken.None));
